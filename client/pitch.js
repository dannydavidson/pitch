...
Meteor.startup(function() {
  ...
  var setHeights = true;
  var matchSession = /^([a-zA-Z]*)$/gi;
  var path = window.location.pathname.substring(1);
  var sessionName = _(matchSession.exec(path)).first();
    ...
    .extend({
      setContentHeight: function() {
        ...
        if (setHeights) {
          ...
          $(".content")
            .height(height - (contentMargin * 2) - headerHeight);

          $('.detail-content').each(function() {
            var $content = $(this);
            var $meta = $content.siblings('.meta');
            var $controls = $content.siblings('.controls');
            var metaOffsetWithHeight = $meta.position().top + $meta.height();
            var controlsOffset = $controls.position().top;
            $content.height(controlsOffset - metaOffsetWithHeight);
          })
        }
      },
      ...
      showStateChange: function(id, fields) {
        ...
        try {
          if (active) {
            var activeEl = $('[data-id="' + active.id + '"]')
              .first(),
      ...
      setActive: function(evt, collectionName) {
        var active,
          ...
          .parents('[data-collection="' + collectionName + '"]')
          .first(),
          id = root.data('id');
      ...
      modifyPanelState: function(id, fields) {
        ...
        content.find('.panel.' + fields.active)
          .addClass('active');
        content.find('.controls span.' + fields.active)
          .addClass('selected');
      ...
      updateObjective: function(evt) {
        var el = $(evt.currentTarget),
          ...
          newDescription = _.str.clean(el.text()
            .replace(/(\r\n|\n|\r)/g, ''));
  ...
  $(window)
    .resize(_(Meteor.pitch.setContentHeight)
      .throttle(200));
...
Template.nav.events({
  ...
  'click li': function(evt) {
    var key = $(evt.currentTarget)
      .data('key'),
      first = _(Meteor.db[key].find({})
        .fetch())
      .first();
